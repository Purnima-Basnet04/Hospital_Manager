...
public class DoctorCancelAppointmentServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        System.out.println("DoctorCancelAppointmentServlet: doGet called");

        // Check if user is logged in
        if (session == null || session.getAttribute("username") == null) {
            System.out.println("DoctorCancelAppointmentServlet: User not logged in, redirecting to login");
        ...
        if (!"doctor".equals(role)) {
            System.out.println("DoctorCancelAppointmentServlet: User is not a doctor, redirecting to index");
        ...
        System.out.println("DoctorCancelAppointmentServlet: userId from session: " + userId);
        
        // If userId is null, try to get it from the user object
        if (userId == null && session.getAttribute("user") != null) {
            User user = (User) session.getAttribute("user");
            userId = user.getId();
            System.out.println("DoctorCancelAppointmentServlet: userId from user object: " + userId);
        ...
        if (userId == null) {
            System.out.println("DoctorCancelAppointmentServlet: User ID not found in session or user object");
            response.sendRedirect(request.getContextPath() + "/login");
            return;
        }
        ...
        if (appointmentIdStr == null || appointmentIdStr.isEmpty()) {
            System.out.println("DoctorCancelAppointmentServlet: No appointment ID provided");
        ...
        try {
            ...
            if (appointment == null) {
                System.out.println("DoctorCancelAppointmentServlet: Appointment not found");
            ...
            if (appointment.getDoctorId() != userId) {
                System.out.println("DoctorCancelAppointmentServlet: Appointment does not belong to the logged-in doctor");
            ...
            if ("Cancelled".equalsIgnoreCase(appointment.getStatus()) || "Completed".equalsIgnoreCase(appointment.getStatus())) {
                System.out.println("DoctorCancelAppointmentServlet: Appointment is already " + appointment.getStatus());
            ...
            if (success) {
                System.out.println("DoctorCancelAppointmentServlet: Appointment cancelled successfully");
                ...
                System.out.println("DoctorCancelAppointmentServlet: Failed to cancel appointment");
            ...
            System.out.println("DoctorCancelAppointmentServlet: Invalid appointment ID format");
            session.setAttribute("errorMessage", "Invalid appointment ID format");
            response.sendRedirect(request.getContextPath() + "/doctor/appointments");
        } catch (Exception e) {
            System.out.println("DoctorCancelAppointmentServlet: Error cancelling appointment: " + e.getMessage());
            e.printStackTrace();
