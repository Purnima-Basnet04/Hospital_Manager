...
@WebServlet("/doctor/better-edit-form")
public class BetterEditFormServlet extends HttpServlet {
    private static final long serialVersionUID = 1L;
       
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        System.out.println("BetterEditFormServlet: doGet method called");
        ...
        System.out.println("BetterEditFormServlet: Record ID = " + recordIdStr);
        
        if (recordIdStr == null || recordIdStr.isEmpty()) {
            System.out.println("BetterEditFormServlet: No medical record ID provided");
        ...
        try {
            ...
            if (medicalRecord == null) {
                System.out.println("BetterEditFormServlet: Medical record not found");
            ...
            if (medicalRecord.getRecordType() == null || medicalRecord.getRecordType().isEmpty()) {
                ...
                System.out.println("BetterEditFormServlet: Set default record type to 'visit'");
            }
            
            if (medicalRecord.getStatus() == null || medicalRecord.getStatus().isEmpty()) {
                medicalRecord.setStatus("Active");
                System.out.println("BetterEditFormServlet: Set default status to 'Active'");
            }
            ...
            System.out.println("BetterEditFormServlet: Forwarding to /doctor/better-edit-form.jsp");
            
            // Forward to edit page
            RequestDispatcher dispatcher = request.getRequestDispatcher("/doctor/better-edit-form.jsp");
            dispatcher.forward(request, response);
            
        } catch (NumberFormatException e) {
            System.out.println("BetterEditFormServlet: Invalid medical record ID format");
            request.setAttribute("errorMessage", "Invalid medical record ID format");
            RequestDispatcher dispatcher = request.getRequestDispatcher("/doctor/medical-records");
            dispatcher.forward(request, response);
        } catch (Exception e) {
            System.out.println("BetterEditFormServlet: Error retrieving medical record: " + e.getMessage());
            e.printStackTrace();
